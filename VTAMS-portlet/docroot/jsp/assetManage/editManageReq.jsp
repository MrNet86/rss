<%@page import="com.viettel.ams.core.service.persistence.AssetManageReqFinderUtil"%>
<%@page import="com.viettel.ams.core.service.persistence.AssetManageReqEntityUtil"%>
<%@page import="com.viettel.ams.core.model.AssetManageReqEntity"%>
...
MerEntityExt mer = null;
if(manageReq != null) {
	List lst = AssetManageReqLocalServiceUtil.findEntityByAssetManageReq(manageReq.getManageReqId());
	for (Object item : lst) {
		Object[] obj = (Object[]) item;
		mer = new MerEntityExt();
		mer.setManageReqId((Long)obj[0]);
		System.out.println("merEntity: "+(Long)obj[1]);
		mer.setMerEntityId((Long)obj[1]);
		mer.setMerName((String)obj[2]);
		mer.setMerCode((String)obj[3]);
		mer.setSerialNumber((String)obj[4]);
		mer.setCount((Double)obj[5]);
		mer.setReqCount((Double)obj[6]);
		mer.setReqDescription((String)obj[7]);

		merEntityExts.add(mer);
	}
}
...
<%-- 	<aui:input name="manageReqId" type="hidden" value="<%= manageReq.getManageReqId() %>"/> --%>
	<aui:input name="manageReqId" type="hidden"/>
	...
	<aui:row>
		<aui:col span="6">
			<aui:input type="text" name="document" label="Req.document"
				cssClass="size60" inlineLabel="false">
				<aui:validator name="required"/>
			</aui:input>
		</aui:col>
		<aui:col span="6">
			<aui:input type="text" name="code" label="Req.code" disabled="true"
				cssClass="size60" inlineLabel="false">
				<aui:validator name="required"/>
			</aui:input>
		</aui:col>
	...
	<liferay-ui:panel collapsible="<%= true %>" extended="<%= true %>" id="selectMerEntityPanel" persistState="<%= true %>" title="Req.MerEntity.Title">
		...
		>
			...
			>
				...
				<liferay-ui:search-container-column-jsp path="/jsp/merEntity/reqCount.jsp" name="Mer.reqCount"/>
				<liferay-ui:search-container-column-jsp path="/jsp/merEntity/description.jsp" name="Common.Comment"/>
